...
NUMERIC_FIELDS = [v for q, v in QUESTION_TO_CANONICAL.items() if 5 <= q <= 17]
...
def _read_dataframe(contents: str, filename: str) -> pd.DataFrame:
    if "," not in contents:
        raise ValueError("The uploaded file payload is not a valid Dash upload.")
    ...
    if ext == "csv":
        for encoding in ("utf-8-sig", "utf-8", "cp1252", "latin-1"):
            try:
                ...
                continue
        raise ValueError("The CSV could not be decoded as text.")
    if ext in {"xlsx", "xls"}:
...
def _clean_text(series: pd.Series) -> pd.Series:
    return series.astype("string").str.strip().replace({"<NA>": pd.NA, "nan": pd.NA, "None": pd.NA})
...
def _column_mapping(raw_df: pd.DataFrame) -> tuple[dict[str, str], list[int]]:
    ...
    missing = [q for q in REQUIRED_QUESTIONS if q not in q_to_col]
    return mapping, missing


def prepare_dataframe(raw_df: pd.DataFrame) -> tuple[pd.DataFrame, ValidationReport, dict[str, str]]:
    if raw_df is None or raw_df.empty:
        return pd.DataFrame(), ValidationReport(False, "The uploaded file contains no rows."), {}

    raw_df = raw_df.copy()
    raw_df.columns = [_norm_header(c) for c in raw_df.columns]
    ...
    if missing:
        return (
            ...
            ValidationReport(
                ok=False,
                message="Required Google Forms questions are missing.",
    ...
    timestamp_col = next((c for c in raw_df.columns if _norm_header(c).lower().startswith("timestamp")), None)
    canonical["timestamp"] = pd.to_datetime(raw_df[timestamp_col], errors="coerce") if timestamp_col else pd.NaT
    ...
    for field in ["age_group", "gender", "programme", "visit_frequency", "mobile_data_frequency"]:
        canonical[field] = _clean_text(canonical[field])

    invalid_numeric: dict[str, int] = {}
    for field in NUMERIC_FIELDS:
        numeric = pd.to_numeric(canonical[field], errors="coerce")
        invalid = canonical[field].notna() & numeric.isna()
        out_of_range = numeric.notna() & ~numeric.between(1, 5)
        invalid_count = int((invalid | out_of_range).sum())
        if invalid_count:
            invalid_numeric[field] = invalid_count
        canonical[field] = numeric.astype("Float64")

    for field in ["improvement_areas", "mobile_data_reasons"]:
        canonical[field] = canonical[field].apply(_clean_multiselect)
    canonical["feedback"] = _clean_text(canonical["feedback"]).fillna("")

    canonical["response_id"] = [f"R-{i:03d}" for i in range(1, len(canonical) + 1)]

    duplicate_rows = int(raw_df.duplicated().sum())
    missing_values = {
        FRIENDLY_NAMES[field]: int(canonical[field].isna().sum())
        for field in CANONICAL_FIELDS
        if field in canonical.columns and int(canonical[field].isna().sum()) > 0
    }
    warnings: list[str] = []
    if duplicate_rows:
        warnings.append(f"{duplicate_rows} exact duplicate row(s) detected; retained for transparency.")
    if invalid_numeric:
        warnings.append("Some satisfaction responses are outside the 1–5 scale or non-numeric.")
    if not canonical["timestamp"].notna().all():
        warnings.append("Some timestamps could not be parsed; the affected responses remain included.")

    ok = not bool(invalid_numeric)
    message = "Valid Survey Format" if ok else "Dataset contains invalid Likert values."
...
def load_uploaded_contents(contents: str, filename: str) -> tuple[pd.DataFrame, ValidationReport, dict[str, str]]:
    raw = _read_dataframe(contents, filename)
    return prepare_dataframe(raw)


def dataframe_from_json(data: str | None) -> pd.DataFrame:
    if not data:
        return pd.DataFrame()
